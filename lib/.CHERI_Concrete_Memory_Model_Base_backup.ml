...
and sub
  x0 x1 = match x0, x1 with
    ...
    | One, One -> Zero_int
and minus_inta k l = match k, l with Neg m, Neg n -> sub n m
...
let rec or_int
  k l = (if Arith.equal_inta k (Arith.uminus_int Arith.one_inta) ||
              Arith.equal_inta l (Arith.uminus_int Arith.one_inta)
          then Arith.uminus_int Arith.one_inta
...
let rec and_int
  k l = (if Arith.equal_inta k Arith.Zero_int ||
          ...
          then Arith.Zero_int
          else (if Arith.equal_inta k (Arith.uminus_int Arith.one_inta) then l
                 else (if Arith.equal_inta l (Arith.uminus_int Arith.one_inta)
                        then k
                        else Arith.plus_inta
                               (Arith.times_inta
                                 (Arith.modulo_inta k
...
module Extended_Nat : sig
  type enat = Enat of Arith.nat | Infinity_enat
end = struct
  ...
  val encode_cap :
    unit capability_ext pre_capability_ext ->
      Numeral_Type.num1 Numeral_Type.bit0 Numeral_Type.bit0 Numeral_Type.bit0
        ...
        Numeral_Type.bit0
        Word.word
  val root_cap_init : unit root_capability_ext pre_capability_ext
  ...
  val tag_update :
    (bool -> bool) ->
      'a capability_ext pre_capability_ext ->
        'a capability_ext pre_capability_ext
  val addr_update :
    (Arith.nat -> Arith.nat) -> 'a pre_capability_ext -> 'a pre_capability_ext
end = struct
...
type 'a root_capability_ext = Root_capability_ext of Extended_Nat.enat * 'a;;
...
let root_cap_init : unit root_capability_ext pre_capability_ext
  = Pre_capability_ext
      (true, true, true, true, Arith.Zero_nat, Arith.Zero_nat,
        Root_capability_ext (Extended_Nat.Infinity_enat, ()));;
...
let rec addr_update
  addra (Pre_capability_ext
          (perm_load, perm_cap_load, perm_store, perm_cap_store, base, addr,
            more))
    = Pre_capability_ext
        (perm_load, perm_cap_load, perm_store, perm_cap_store, base, addra addr,
          more);;
...
type 'a heap_ext =
  Heap_ext of
    unit Fat_Capability.root_capability_ext Fat_Capability.pre_capability_ext *
      (Arith.nat,
        Numeral_Type.num1 Numeral_Type.bit0 Numeral_Type.bit0 Numeral_Type.bit0
          Word.word)
        Mapping.mapping *
...
let rec typ_offset t a = Arith.modulo_nat a (sizeof t);;
...
let rec allocate_p
  h t = (if not (allocd_region h (Fat_Capability.addr (curr h))
                  (Arith.plus_nat
                    (Arith.plus_nat (Fat_Capability.addr (curr h)) (sizeof t))
                    (typ_offset t (Fat_Capability.addr (curr h))))
                  false)
          then None
          else Some (gamma_update
                       (fun _ ->
                         map_alloc (gamma h)
                           (Arith.plus_nat (Fat_Capability.addr (curr h))
                             (typ_offset t (Fat_Capability.addr (curr h))))
                           (sizeof t))
                       (curr_update
                         (fun _ ->
                           Fat_Capability.addr_update
                             (fun _ ->
                               Arith.plus_nat
                                 (Arith.plus_nat (Fat_Capability.addr (curr h))
                                   (sizeof t))
                      ...
                      Fat_Capability.Pre_capability_ext
                        (Fat_Capability.perm_load (curr h), false,
                          Fat_Capability.perm_store (curr h), false,
                          Arith.plus_nat (Fat_Capability.addr (curr h))
                            (typ_offset t (Fat_Capability.addr (curr h))),
                          Arith.plus_nat (Fat_Capability.addr (curr h))
                            (typ_offset t (Fat_Capability.addr (curr h))),
  ...
  h t = (if not (allocd_region h (Fat_Capability.addr (curr h))
          ...
          then None
                       ...
                       (curr_update
                         ...
                         h),
                      Fat_Capability.Pre_capability_ext
                        (Fat_Capability.perm_load (curr h),
                          Fat_Capability.perm_cap_load (curr h),
                          Fat_Capability.perm_store (curr h),
                          Fat_Capability.perm_cap_store (curr h),
                          Arith.plus_nat (Fat_Capability.addr (curr h))
                            (typ_offset t (Fat_Capability.addr (curr h))),
                          Arith.plus_nat (Fat_Capability.addr (curr h))
                            (typ_offset t (Fat_Capability.addr (curr h))),
...
let rec allocate
  h t = (match t with MSpec_Library.I8 -> allocate_p h t
          | MSpec_Library.I64 -> allocate_p h t
          | MSpec_Library.Ptr _ -> allocate_c h t);;

let heap_init : unit heap_ext
  = Heap_ext
      (Fat_Capability.root_cap_init,
          ...
          (Type_Length.len_bit0
            (Type_Length.len_bit0
              (Type_Length.len_bit0
                (Type_Length.len_bit0
                  (Type_Length.len_bit0 Type_Length.len_num1))))))
        (Arith.Pos (Arith.Bit0 (Arith.Bit1 (Arith.Bit0 Arith.One)))));;
