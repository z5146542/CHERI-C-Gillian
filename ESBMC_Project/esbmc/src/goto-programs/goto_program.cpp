  ...
  bool show_location) const
{
  ...
  case GOTO:
    if(!is_true(guard))
    {
      std::string lines = from_expr(ns, identifier, guard);
  ...
  case FUNCTION_CALL:
  {
    std::string output = from_expr(ns, "", migrate_expr_back(code));
  ...
  case ASSIGN:
  {
    std::string output = from_expr(ns, identifier, code);
  ...
  case END_FUNCTION:
    {
      ...
      if(from_type(ns, symbol->id, symbol->type).find("void") != std::string::npos)
        out << "ret := undefined;" << "\n        ";
    }
    out << "return\n};";
    if(show_location) {
