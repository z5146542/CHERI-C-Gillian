...
text \<open>In this subsection, we provide the type and value system used by the CHERI-C Memory Model.
      ...
      their normal representation and list of bytes so that storing values work as intended.\<close>
...
text \<open>We first formalise the capability type. We first define \textit{memory capabilities} as a record,
      then we define \textit{tagged capabilities} by extendeding the record. Isabelle provides 
      truncating and extending functions with proofs that will come into play when loading and storing
      capabilites from and to memory. We state the class comp\_countable, but countable is sufficient.\<close>
...
text \<open>cctupe corresponds to $\tau$ in the paper, where $\tau$ is the type system.\<close>
datatype cctype = 
  Uint8
  | Sint8
  | Uint16
...
text \<open>$'a ccval$ corresponds to $\mathcal{V}_\mathcal{C}$ in the paper. $'a$ in this instance must be countable. \<close>
datatype 'a ccval = 
  Uint8_v      "8 word"
  | Sint8_v    "8 sword"
  | Uint16_v   "16 word"
...
text \<open>In this subsection, we provide proofs for the conversion between words and list of words.
      We also provide proofs that casting primitive values is correct.\<close>
...
lemma integer_word_bounded_eq:
  ...
  using assms 
  by (metis integer_less_eq_iff integer_of_int_eq_of_int minus_integer.rep_eq of_int_0_le_iff 
      of_int_eq_1_iff of_int_eq_numeral_power_cancel_iff of_int_integer_of word_of_int_inverse 
      zle_diff1_eq)
...
lemma integer_sword_bounded_eq:
  ...
  by (smt (verit) diff_numeral_special(11) int_of_integer_numeral integer_less_eq_iff 
      integer_of_int_eq_of_int len_signed minus_integer.rep_eq numeral_power_eq_of_int_cancel_iff 
      of_int_integer_of of_int_power_less_of_int_cancel_iff one_integer.rep_eq sint_of_int_eq 
...
text \<open>cast\_val is an executable code that ensures easy casting of values.\<close>
