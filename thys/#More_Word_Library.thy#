...
(*
fun sizeof :: "cctype \<Rightarrow> nat"
  where
  "sizeof \<tau> = (case \<tau> of
     Uint8  \<Rightarrow> 1
   | Sint8  \<Rightarrow> 1
   | Uint16 \<Rightarrow> 2
   | Sint16 \<Rightarrow> 2
...
lemma "sizeof \<tau> = x \<Longrightarrow> \<exists> n. 2 ^ n = x"
  apply (simp split: cctype.split_asm)
          apply fastforce+
